...
class NPUModelRunner(GPUModelRunner):
    ...
    def _pad_query_start_loc_for_fia(self, num_tokens_padded: int, num_reqs_padded: int, num_reqs: int):
        """
        ...
        if num_tokens_padded == num_reqs_padded * self.uniform_decode_query_len:
            ...
            self.query_start_loc.np[num_reqs + 1:num_reqs_padded] = (self.arange_np[1:num_reqs_padded + 1 - num_reqs]
                                                    * self.uniform_decode_query_len + last_loc)
        else:
    ...
    def _build_attention_metadata(
        ...
        assert self.query_start_loc.np[num_reqs_padded] == num_tokens_padded, f"Due to FIA kernel constraints, when the layout is TND the first dimension of `hidden_states` ({num_tokens_padded}) must equal the last element of `actual_seq_lengths_q` ({self.query_start_loc.np[num_reqs_padded]})."
